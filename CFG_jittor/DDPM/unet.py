...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):    
    def execute(self, x, t_emb, c_emb, mask):
        for layer in self:
...
class AttentionBlock(nn.Module):
    ...
    def execute(self, x):
        ...
        attn = jt.einsum("bct,bcs->bts", q * scale, k * scale)
        attn = nn.softmax(attn, dim=-1)
        h = jt.einsum("bts,bcs->bct", attn, v)
        h = h.reshape(B, -1, H, W)
        h = self.proj(h)
        return h + x
...
class UnetModel(nn.Module):
    ...
    def execute(self, x, timesteps, c, mask):
        """
